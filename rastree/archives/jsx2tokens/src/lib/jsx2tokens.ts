// import { TypeToken, jsxTokenize } from './jsxTokenize'

// export const jsx2tokens = <C extends object>(
//   source: string, insideJSX?: boolean,
//   cb?: (v: TypeToken, k: number, a: TypeToken[], ctx: C) => boolean | void, ctx?: C
// ): TypeToken[] => jsxTokenize(source, insideJSX, cb, ctx).tokens

import { isChildlessTagName } from '@rastree/utilites'

export declare type TypeRange = [number, number]
export declare type TypeSourcePosition = { line: number; column: number }
...
export const jsx2tokens = (() => {
  ...
  type TypeIam = {
    ...
    isBreakLoop: boolean
    readonly callback: TypeJscTokenizeCallback
  ...
  // let source: string
  // let isBreakLoop: boolean
  // let callback: TypeJscTokenizeCallback
  // let cbCtx: object

  // let tokens: TypeToken[]

  // let tokenLast: TypeToken | null
  // let tagNameLast: string
  // let deep: number

  // let idx: number
  // let line: number
  // let rangeStart: number// , lineStart: number//, columnStart: number
  ...
  const runCallback = (iam: TypeIam): void => {
    iam.isBreakLoop = !!iam.callback(iam.tokenLast!, iam.tokens.length - 1, iam.tokens, iam.cbCtx)
  }
            ...
            case '>'/* > >= >> >>= >>> >>>= */:
              switch (iam.ENV) {
                case '%<>%':
                  ...
                  if (iam.tagNameLast === 'script' || iam.tagNameLast === 'style') {
                    createPunctuator(iam, 0, TOKEN_JSX_TAG_OPENER_END)
                    iam.deep++
                    env(iam, ['%script%', iam.tagNameLast])
                  } else if (
                    !/^[!?%]/.test(iam.tagNameLast) && !isChildlessTagName(iam.tagNameLast)
                  ) {
                    createPunctuator(iam, 0, TOKEN_JSX_TAG_OPENER_END)
                    iam.deep++
                    env(iam, '%><%')
  ...
  return <C extends readonly [] | {} | undefined = undefined>(
    _source: string,
    insideJSX?: boolean,
    cb?: (v: TypeToken, k: number, a: TypeToken[], ctx: C) => boolean | void,
    ctx?: C
  ) => {
    const ENV = insideJSX ? '%><%' : ''
    const iam: TypeIam = {
