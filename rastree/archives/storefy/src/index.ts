...
//   TOKEN_PUCNTUATOR, trimCircleBrackets
...
import {
  jsx2tokens, TypeToken,
  stringifyTokens, trimTokens, prevRealTokenIndex,
  TOKEN_PUCNTUATOR, trimCircleBrackets,
  TOKEN_IDENTIFIER,
  TOKEN_SPACE, TOKEN_COMMENT_LINE, TOKEN_COMMENT_BLOCK
  // TOKEN_TEMPLATE_HEAD, TOKEN_TEMPLATE_MIDDLE, TOKEN_MODIFIER
  // TOKEN_KEYWORD, TOKEN_MODIFIER, TOKEN_JSX_EXPRESSION_START
} from '@rastree/jsx2tokens'

const isNot2 = (token: TypeToken | null): boolean =>
  !token || token.type === TOKEN_PUCNTUATOR && /[^.})\]!]$/.test(token.value)
  // token.type === TOKEN_TEMPLATE_HEAD || token.type === TOKEN_TEMPLATE_MIDDLE ||
...
const __getStoreValue__ = (
  ...
  for (let token2: TypeToken, deep2 = deep; i-- > 0;) {
    ...
    if (deep === deep2) {
      if (token2.type === TOKEN_PUCNTUATOR && !/^[.})\]!]$/.test(token2.value)
      // token.type === TOKEN_TEMPLATE_HEAD || token.type === TOKEN_TEMPLATE_MIDDLE ||
      // token.type === TOKEN_MODIFIER
      ) {
        break
  ...
  while (tokens[i] && (tokens[i].value === '!' ||
    tokens[i].type === TOKEN_SPACE ||
    tokens[i].type === TOKEN_COMMENT_LINE ||
    tokens[i].type === TOKEN_COMMENT_BLOCK)) i++
...
export const storefyExpression = (
  source: string, rease: string
): [string, number] => {
  const identifiers: any = {}
  const tokens = trimTokens(jsx2tokens(source, false, (token) => {
...
export const storefy = (
  source: string, rease: string = '_$'
): string => {
  // console.log(source)
  let i = 1
  let isStore = 0
  let res: [string, number] = [source, isStore]
  while ((res = storefyExpression(res[0], rease))[1] && i++ < 99) {
...
export const isStorefy = (v: any): boolean =>
  (v += '').indexOf(STOREFY_COMMENT_MARKER_STRICT) === 0

export const isStorefyForExpo = (v: any): boolean =>
  (v += '').indexOf(STOREFY_COMMENT_MARKER_MAYBE) === 0 || isStorefy(v)
