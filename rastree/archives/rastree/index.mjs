...
import { validateCid, createScoper, isValidVariable } from './lib/utils';
import { style } from './core/style';
import { template } from './core/template';
import { extractImports, mergeImports, __REASE_COMPONENT__ } from './core/script/lib/extractImports';
...
var __createComponent__ = ({
  ...
  var __d__ = getDelimeter(code + module + name);
...
var rastree = (source, {
  ...
  var {
    options,
    sections
  } = isString(source) ? parseComponent(source) : source;
  var {
    script,
    style: style$1,
    template: template$1
  ...
  var cid = options.cid = validateCid(options.cid);
  var [scoper] = createScoper(cid, options['css-scope-type']); // console.log('scoper', [cid, scoper, scoperInject])
  // console.log(style)
  // const styles = []
  ...
  cssExternal = trimLeft(cssExternal); // console.log(styles)
  // console.log(template)

  var templateClientCodeDirty = ''; // let templateServerCodeDirty = ''

  forEachLeft(template$1, ({
    attributes,
    code
  }) => {
  ...
  templateClientCodeDirty = trim(templateClientCodeDirty); // templateServerCodeDirty = trim(templateServerCodeDirty)

  console.log('templateClientCodeDirty\n', [templateClientCodeDirty]);
  var templateClientCode = template(templateClientCodeDirty).toString();
  ...
  forEachLeft(script, ({
    ...
    var {
      imports,
      code
    } = extractImports(codeDirty); // console.log({ imports, code })

    if (client) {
      scriptClient[module ? 'module' : 'code'] += code;
      mergeImports(scriptClient.imports, imports);
    }

    if (server) {
      scriptServer[module ? 'module' : 'code'] += code;
      mergeImports(scriptServer.imports, imports);
    }
  });
