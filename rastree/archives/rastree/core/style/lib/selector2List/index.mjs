...
var fixList = arr => {
  ...
  while (++i < arr.length) {
    ...
    if (node0 && node0[1] === 2 && (!i || !node1 || node1[1] === 2 && (node1[2] = trim((node0[2] === ',' ? ',' : '') + node1[2] + node0[2])[0] || ' '))) {
      arr.splice(i--, 1); // , (i = i || 1)
    } else if (node0 && node1 && (rtype = node0[3]) && node1[2] === '(') {
      node0[3] = ''; // eslint-disable-next-line no-loop-func

      findIndexLeft(arr, v => !!(v[0] === node0[0] && v[2] === ')' && (v[3] = v[3] || rtype)), i);
...
var joinListDirtyArr = arr => {
  ...
  while (prev.length) {
    ...
    forEachLeft(clone(lastOrigin), last => {
      res.push(last);
      i = last.length;

      while (i--) {
        // eslint-disable-next-line no-loop-func
        if (last[i][2] === '&') last.splice(i, 1, ...clone(prevNodes).map(v => (v[0] += last[i][0], v)));
      }
    });
  }
...
export { GLOBAL, SCOPED, STATED, listDirty2List, selector2List, selector2ListDirty };
