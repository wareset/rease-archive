...
class CssProgram extends ProgramDirty {
  constructor(tokens) {
    super();
    forEachLeft(tokens, v => {
  ...
  static toCode(arr, options) {
    ...
    forEachLeft(arr, v => {
      ...
      /* COMMENT */
        ...
        var __t1__ = repeat(IND, __l1__);

        var COMMA = v[0] === 0
        /* COMMENT */
        ? '' : CSP;
        var attr = CRLF + __t1__ + (isArray(v[2]) ? v[2].join(SEMI) : v[2]) + COMMA;
        start = '';
        space = '';
        is = false;
        var cache = [];
        if (!v[1].length) res += repeat(FBC, lk);
        forEachLeft(v[1], (v2, k2) => {
    ...
    res += repeat(FBC, lk);
    res = trim(res) + CRLF; // const { global, scoper } = options
...
export { CSS_PROGRAM_TO_CODE_OPTIONS, CssProgram };
