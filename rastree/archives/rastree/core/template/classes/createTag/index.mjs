...
import { jsx2tokens, stringifyTokens, trimCircleBrackets, TOKEN_KEYWORD } from '@rastree/jsx2tokens';
import { isLiteral, offset } from '../utils';
...
var isExpression = v => {
  if (v[0] === '{' && last(v) === '}') {
    try {
      return jsx2tokens(v).tokens.slice(1, -1).every(v => v.deep);
    } catch (e) {
      /* */
    }
  }
...
var fixArrayValues = arr => {
  ...
  arr.forEach(v => {
    if (v) {
      if (!islit) islit = isLiteral(v), data.push(v);else {
        (islit = isLiteral(v)) ? data[data.length - 1] = jsonStringify(jsonParse(last(data)) + jsonParse(v)) : data.push(v);
      }
    }
  });
  return data;
...
var normalizeSysValue = (value, salt) => storefyExpression(jsx2tokens(isExpression(value) ? value.slice(1, -1) : jsonStringify(value)).tokens, salt);
...
var normalizeValue = (value, salt) => {
  ...
  if (value.indexOf('{') < 0 || value.indexOf('}') < 0) {
    ...
    var tokens = jsx2tokens(value).tokens;
    res = [];
    var token;
...
var parseTagAttributes = (tokens, salt) => {
  var arr = parseAttributes(tokens).map(v => normalizeAttributes(v));
  var tag;
  var flags = {};
  var props = {
    client: {},
  ...
  arr.forEach(([name, value]) => {
    ...
    if (matches = /^r?:tag.?(?:Name)?(?:\(([^]+)\))?$/i.exec(name)) {
      if (matches[1]) sys.tag = storefyExpression(matches[1], salt);else if (value) sys.tag = normalizeSysValue(value, salt);
      return;
    } // SLOT


    if (matches = /^r?:slot(?:\(([^]+)\))?$/i.exec(name)) {
      if (matches[1]) sys.slot = storefyExpression(matches[1], salt);else if (value) sys.slot = normalizeSysValue(value, salt);
      return;
    } // NAME


    if (matches = /^r?:name(?:\(([^]+)\))?$/i.exec(name)) {
      if (matches[1]) sys.name = storefyExpression(matches[1], salt);else if (value) sys.name = normalizeSysValue(value, salt);
      return;
    } // VARS


    if (matches = /^r?:(?:var|let)s?(?:\(([^]+)\))?$/i.exec(name)) {
      var vars = stringifyTokens(trimCircleBrackets(jsx2tokens(fixIfExpression(matches[1] || value)).tokens));

      if (vars) {
        var args = [];
        var vals = [];
        parseAttributes(jsx2tokens(vars).tokens).forEach(v => {
          args.push(v[0]);
    ...
    if (matches = /^r?:el(?:se)?.?if(?:\(([^]+)\))?$/i.exec(name)) {
      if (matches[1]) value = storefyExpression(matches[1], salt);else if (value) value = normalizeSysValue(value, salt);
    ...
    if (matches = /^r?:(?:for(?:each)?|each)(?:\(([^]+)\))?$/i.exec(name)) {
      ...
      var _tokens = trimCircleBrackets(jsx2tokens(value).tokens);

      if (_tokens.length) {
        var _args = '';
        var end = findIndexLeft(_tokens, v => v.value === 'in' && v.type === TOKEN_KEYWORD);

        if (end > -1) {
          _args = stringifyTokens(trimCircleBrackets(_tokens.splice(0, end)));
        }

        var each = storefyExpression(_tokens.slice(1), salt);
    ...
    if (matches = /^(?:(c|s|b|client|server|browser)\W+)?(.+)$/i.exec(name)) {
      ...
      if (name.indexOf(':') > -1) {
        ...
        if (!isNaN(+_arr[1])) name = trim(_arr[0]);
      }
  ...
  for (var env in props) {
    ...
    for (var a = keys(propsEnv), i = a.length; i-- > 0;) {
      var key = a[i];

      var _arr2 = fixArrayValues(concat(...propsEnv[key].map(v => v ? normalizeValue(v, salt) : 'true')));

      if (!_arr2.length) delete propsEnv[key];else if (_arr2.length === 1) propsEnv[key] = _arr2[0];else propsEnv[key] = STOREFY + salt + ".c([" + _arr2.join(', ') + "])"; // если комплексный массив, то добавить к нему ключ '%complex%' например
    }
...
var createTag = ({
  ...
  var usesStr = '';
  var usesStrTemp = '';
  usesStrTemp = uses.map(v => !v[1] || !env || v[1] === env ? v[0] : '').filter(Boolean).join(', ');
  ...
  var attrs = trimRight([sys.tag, propsStr, usesStr, nameStr].join(', '), ',\\s');
...
export { createTag, fixArrayValues, parseTagAttributes };
