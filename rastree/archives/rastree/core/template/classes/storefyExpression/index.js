...
var __getStoreValue__ = (tokens, last, stores, salt) => {
  ...
  for (var token2, deep2 = deep; i-- > 0;) {
    ...
    if (deep === deep2) {
      if (token2.type === jsx2tokens.TOKEN_PUCNTUATOR && !/^[.})\]]$/.test(token2.value)) break;
    }

    deep2 = token2.deep;
  }

  var store = jsx2tokens.stringifyTokens(tokens.splice(++i, last - i).slice(0, -1)).trim();
...
var storefyExpression = (tokens, salt) => {
  ...
  if (stores.length) {
    ...
    var data = jsx2tokens.stringifyTokens(jsx2tokens.trimCircleBrackets(tokens)).trim();
    if (stores.length === 1 && attrs === data) res = stores[0];else res = STOREFY + salt + ".s([" + stores.join(', ') + "], (" + attrs + ") => (" + data + "))";
  } else {
    res = '(' + jsx2tokens.stringifyTokens(jsx2tokens.trimCircleBrackets(tokens)).trim() + ')';
  }
...
exports.STOREFY = STOREFY;
exports.storefyExpression = storefyExpression;
