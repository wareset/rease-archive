...
import { isStorefyForExpo } from '../storefy'
...
function _hashefy(s: any): number {
  s += ''
  let z1 = 1
  const r = 1 / 9973
  for (let c: number, l = s.length, n = l / 99 | 0 || 1, i = 0; i < l; i += n) {
    if ((c = s.charCodeAt(i)) === 13) i -= n - 1
    else z1 += c * z1 * 997 / (c + z1) + r, z1 > 0 && (z1 -= z1 | 0)
  }
  return z1
}

function createHASH(s: any): string {
  let h = _hashefy(s).toString(36).slice(2) || '0'
  for (;h.length < 9;) h += h
  h = h.slice(0, 9)
  if (/\d/.test(h[0])) h = 'h' + h.slice(0, 8)
  return h
}
      ...
      if (c[0] === '(' || c[0] === '[') cur.push(c)
        ...
        for (let a = c.split(/(&)|\s*([|>+~\s])\s*/), i = -1; ++i < a.length;) {
          if (a[i]) {
            ...
            else c = c.replace(/\.?\$(\w+)/g, '%REASE%SELECTOR%$1')
...
export function styles(
  source: string, hash: string, _cssAlias: string, storefy: Function
): any {
  // console.log(source)
  ...
  let n = 0
  let _s = '_s'
  while (_s + n in identifiers) n++
  _s += n
  let _p = '_p'
  while (_p + n in identifiers) n++
  _p += n
  ...
  for (const k in REPLACERS) {
    if (!isStorefyForExpo(v = storefy(REPLACERS[k]))) {
      REPLACERS[k] = '(' + v + ')!!'
    }
  }
  ...
  // let getters = ''
  // for (const k in REASE_SELECTORS_CACHE) {
  //   getters += `  get ${k}() { return this._(${REASE_SELECTORS_CACHE[k]}) },\n`
  // }
